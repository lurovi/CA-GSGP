...
class MultiProcessingParallelizer(Parallelizer):
    ...
    def __init__(self,
        ...
        if self.num_workers() > mp.cpu_count():
            raise AttributeError(
                f"Specified a number of cores ({self.num_workers()}) that is greater than the number of cores supported by your computer ({mp.cpu_count()}).")
    ...
    def parallelize(self, target_method: Callable, parameters: list[dict[str, Any]]) -> list[T]:
        ...
        number_of_processes: int = {-2: (mp.cpu_count()), -1: (mp.cpu_count() - 1)}.get(self.num_workers(), self.num_workers())

        with mp.Pool(processes=number_of_processes, maxtasksperchild=1) as pool:
            map_function: Callable = partial(pool.map, chunksize=self.chunksize())
