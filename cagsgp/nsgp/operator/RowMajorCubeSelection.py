...
class RowMajorCubeSelection(Selection):
    ...
    def _do(self, problem, pop, n_select, n_parents, **kwargs):
        ...
        row_major_cube: RowMajorCube = RowMajorCube(individuals, n_channels=self.__n_channels, n_rows=self.__n_rows, n_cols=self.__n_cols, clone=False, fill_with_none=True)

        S: np.ndarray = np.full((self.__n_channels * self.__n_rows * self.__n_cols, 2), fill_value=None, dtype=object)
        t: int = 0
