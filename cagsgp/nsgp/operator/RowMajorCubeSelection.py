...
class RowMajorCubeSelection(Selection):
    def __init__(self,
        ...
        self.__size: int = self.__n_channels * self.__n_rows * self.__n_cols
        ...
        for l in range(self.__n_channels):
            for i in range(self.__n_rows):
                for j in range(self.__n_cols):
                    competitors: list[tuple[int, Node, np.ndarray]] = row_major_cube.neighborhood([l, i, j], include_current_point=True, clone=False)
